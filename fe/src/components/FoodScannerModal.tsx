      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-lg p-6 shadow-2xl text-white max-h-[90vh] overflow-y-auto relative">
        ...
        <div className="flex bg-slate-800/80 p-1 rounded-2xl mb-4">
          <button
            ...
            className={`flex-1 py-2 text-xs font-semibold rounded-xl flex items-center justify-center space-x-1.5 transition-all ${
              mode === 'image' ? 'bg-emerald-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2 text-xs font-semibold rounded-xl flex items-center justify-center space-x-1.5 transition-all ${
              mode === 'text' ? 'bg-emerald-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white'
            }`}
          >
        ...
        {!result && !isScanning && (
          <div className="space-y-4">
            {mode === 'image' ? (
              <div className="border-2 border-dashed border-slate-700 hover:border-emerald-500/50 rounded-2xl p-6 text-center bg-slate-800/40 relative">
                {imagePreview ? (
                  <div className="relative">
                    <img src={imagePreview} alt="Food preview" className="max-h-48 rounded-xl mx-auto object-cover border border-slate-700" />
                    <button
                  ...
                  <label className="cursor-pointer block py-6">
                    ...
                    <span className="text-xs font-semibold text-slate-200">Chạm để chọn ảnh món ăn từ máy hoặc camera</span>
                    <span className="block text-[10px] text-slate-500 mt-1">Hỗ trợ JPG, PNG, WEBP</span>
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1.5">Mô tả món ăn của bạn bằng ngôn ngữ tự nhiên</label>
                <textarea
                  rows={3}
        ...
        {isScanning && (
          <div className="py-12 text-center space-y-3">
            ...
            <p className="text-sm font-bold text-emerald-400">AI Computer Vision đang bóc tách thực phẩm...</p>
            <p className="text-xs text-slate-400">Ước tính khối lượng (g), Calorie và Macros chuẩn ground-truth</p>
          </div>
        )}
          ...
          <div className="space-y-4 animate-fadeIn">
            ...
            <div className="flex items-center justify-between bg-slate-800/80 p-3 rounded-2xl border border-slate-700">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs font-semibold text-emerald-400">Độ tin cậy AI: {Math.round(result.confidence_score * 100)}%</span>
              </div>
              <button
            ...
            <div className="bg-slate-800/50 p-4 rounded-2xl border border-slate-800 space-y-3">
              <div>
                <label className="block text-[11px] text-slate-400 mb-1">Tên món ăn (Cho phép chỉnh sửa)</label>
                <input
                  type="text"
                  value={editFoodName}
              ...
              <div className="grid grid-cols-3 gap-2 pt-1">
                ...
                <div className="bg-slate-900 p-2 rounded-xl border border-slate-800 text-center">
                  <span className="block text-[10px] text-amber-400 font-semibold">Carbs (Đường)</span>
                  <input
                    type="number"
                    value={editCarbs}
                ...
                <div className="bg-slate-900 p-2 rounded-xl border border-slate-800 text-center">
                  <span className="block text-[10px] text-rose-400 font-semibold">Fat (Chất Béo)</span>
                  <input
                    type="number"
                    value={editFat}
