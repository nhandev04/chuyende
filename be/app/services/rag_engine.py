...
def load_genai_modules():
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"❌ Failed to load google-genai library on backend server. "
                f"Please restart uvicorn (Press CTRL+C and run: python -m uvicorn app.main:app --reload --port 8000). Details: {str(import_err)}"
            )
...
def generate_rag_meal_plan(
    ...
    if not GEMINI_API_KEY:
        raise RuntimeError("❌ GEMINI_API_KEY is not configured in backend environment variables (.env).")
    ...
    try:
        ...
        raise RuntimeError(f"❌ Failed to connect to Google Gemini API: {str(e)}")
