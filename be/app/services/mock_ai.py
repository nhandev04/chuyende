...
def real_analyze_body(height_cm: float, weight_kg: float, age: int, gender: str, goal: str) -> Dict[str, Any]:
    """
...
def mock_analyze_body(height_cm: float, weight_kg: float, age: int, gender: str, goal: str) -> Dict[str, Any]:
    return real_analyze_body(height_cm, weight_kg, age, gender, goal)

def mock_generate_recommendations(consumed_cal: float, target_cal: float, goal: str) -> Dict[str, Any]:
    """Generates personalized diet recommendations using caloric deficit/surplus logic"""
    diff = consumed_cal - target_cal
    
    if diff > 300:
        status = "over_budget"
        advice = f"Bạn đã nạp vượt mức {int(diff)} kcal so with target hôm nay. Hãy đi bộ 30 phút hoặc giảm bớt khẩu phần ăn tối."
        suggested_meals = [
...
def mock_analyze_food(text_prompt: Optional[str] = None, filename: Optional[str] = None) -> Dict[str, Any]:
    """Fallback food lookup dictionary for text and unrecognized images"""
